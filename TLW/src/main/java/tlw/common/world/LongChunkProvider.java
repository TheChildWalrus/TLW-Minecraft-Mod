...
public class LongChunkProvider implements IChunkProvider
{
	private LongWorldProvider provider;
    ...
    public LongChunkProvider(LongWorldProvider worldprovider, World world, long l)
    {
    ...
    public void populate(IChunkProvider ichunkprovider, int i, int j)
    {
        ...
        LongWorldProperties worldProps = LongWorldProperties.getLongProperties(worldObj, provider.stepID);
        ...
        if (rand.nextInt(4) == 0)
        {
			...
			if (j2 < 60 || rand.nextFloat() < worldProps.lakeChance)
			{
        ...
        if (rand.nextInt(8) == 0)
        {
            ...
            if (j1 < 60 || rand.nextFloat() < worldProps.lavaLakeChance)
            {
                (new WorldGenLakes(Blocks.lava)).generate(worldObj, rand, i1, j1, k1);
            }
        }
		
		LongBiomeDecorator.decorate(worldProps, worldObj, rand, k, l);
		
		float animals = worldProps.animalChance;
		while (animals > 1F)
		{
			SpawnerAnimals.performWorldGenSpawning(worldObj, biomegenbase, k + 8, l + 8, 16, 16, rand);
			animals -= 1F;
		}
		if (rand.nextFloat() < animals)
		{
			SpawnerAnimals.performWorldGenSpawning(worldObj, biomegenbase, k + 8, l + 8, 16, 16, rand);
		}
